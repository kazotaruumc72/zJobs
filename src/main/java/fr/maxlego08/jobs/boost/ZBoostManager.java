    ...
    public void createBoost(CommandSender sender, OfflinePlayer offlinePlayer, List<String> jobs, List<JobActionType> actions, List<String> targets, double moneyBoost, int amount, double experienceBoost) {
        ...
        jobManager.loadOfflinePlayer(offlinePlayer.getUniqueId(), playerJobs -> {
            storageManager.createBoost(offlinePlayer.getUniqueId(), jobs, actions, targets, moneyBoost, experienceBoost, amount, boost -> {
                ...
                message(this.plugin, sender, Message.BOOST_CREATE_SUCCESS, "%player%", offlinePlayer.getName(), "%amount%", amount, "%experience%", format(experienceBoost), "%money%", format(moneyBoost));

            }, () -> message(this.plugin, sender, Message.BOOST_CREATE_ERROR));
        });
    }
    ...
    public void removeBoost(CommandSender sender, OfflinePlayer offlinePlayer, int boostId) {
        ...
        jobManager.loadOfflinePlayer(offlinePlayer.getUniqueId(), playerJobs -> {
            ...
            if (!playerJobs.getBoosts().contains(boostId)) {
                message(this.plugin, sender, Message.BOOST_REMOVE_ERROR, "%player%", offlinePlayer.getName(), "%id%", boostId);
                return;
            }
            ...
            message(this.plugin, sender, Message.BOOST_REMOVE_SUCCESS, "%player%", offlinePlayer.getName(), "%id%", boostId);
        });
    }
    ...
    public void showBoosts(CommandSender sender, OfflinePlayer offlinePlayer) {
        ...
        jobManager.loadOfflinePlayer(offlinePlayer.getUniqueId(), playerJobs -> {
            ...
            if (playerJobs.getBoosts().isEmpty()) {
                message(this.plugin, sender, Message.BOOST_SHOW_EMPTY, "%player%", offlinePlayer.getName());
                return;
            }

            var boosts = playerJobs.getBoosts().getBoosts();
            message(this.plugin, sender, Message.BOOST_SHOW_HEADER, "%player%", offlinePlayer.getName(), "%amount%", boosts.size(), "%s%", boosts.size() <= 1 ? "" : "s");
            for (Boost boost : boosts) {
                message(this.plugin, sender, Message.BOOST_SHOW_INFO,
